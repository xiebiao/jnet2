...
public class IoBuffer {
   ...
   * 
   * <p>
   * The byte order is used when reading or writing multibyte values, and when creating buffers that
   * are views of this byte buffer. The order of a newly-created byte buffer is always
   ...
   * 
   * @param bo The new byte order, either {@link ByteOrder#BIG_ENDIAN BIG_ENDIAN} or
   ...
   * new limit. If the mark is defined and larger than the new limit then it is reallocate and
   * reset. </p>
   * 
  ...
  /**
   * Returns the number of elements between the current position and the limit. </p>
   * 
   ...
   * 
   * <p>
   * Invoke this method before a sequence of channel-write or <i>get</i> operations, assuming that
   * the limit has already been set appropriately. For example:
   * 
   * <blockquote>
   * 
   * <pre>
   ...
   * 
   * <p>
   * After a sequence of channel-read or <i>put</i> operations, invoke this method to prepare for a
   * sequence of channel-write or relative <i>get</i> operations. For example:
   * 
   * <blockquote>
   * 
   * <pre>
   ...
   * 
   * <p>
   * This method is often used in conjunction with the {@link java.nio.ByteBuffer#compact compact}
   ...
   * 
   * <p>
   * Invoke this method before using a sequence of channel-read or <i>put</i> operations to fill
   ...
   * 
   * <p>
   * The bytes between the buffer's current position and its limit, if any, are copied to the
   ...
   * <i>n</i>&nbsp;=&nbsp;<tt>limit()</tt>&nbsp;-&nbsp;<tt>1</tt> &nbsp;-&nbsp;<i>p</i>. The
   * buffer's position is then set to <i>n+1</i> and its limit is set to its capacity. The mark, if
   * defined, is discarded.
   * 
   * <p>
   * The buffer's position is set to the number of bytes copied, rather than to zero, so that an
   ...
   * 
   * <p>
   * Invoke this method after writing data from a buffer in case the write was incomplete. The
